...
So "the table only ever grew" would have been the wrong story: in production it never grows, because nothing populates it. `SessionRepository`, the `sessions` table, and its index are unused schema and dead code, left over from before the app settled on native PHP sessions for admin auth. Wiring a prune job into `cron.php` for a table nothing writes to would have been solving a problem that doesn't exist, so no code change shipped here at the time — this was a finding to hand to the maintainer, not a performance fix, and removing the table is a schema change CLAUDE.md requires explicit confirmation for.

**Update:** confirmed correct and acted on independently — [#534](https://github.com/dgloeckner/clubbar/pull/534), "Remove dead SessionRepository and its unused sessions table," landed on `main` reaching the identical diagnosis (native PHP sessions, no DB save handler) and merged migration `044_drop_sessions_table.sql`. Merged into this branch; nothing here conflicted since this branch never touched those files.
