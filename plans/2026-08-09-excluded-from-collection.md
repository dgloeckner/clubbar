...
- [ ] **0.1** New `e2etests/tests/api/member-exclusions.spec.ts` covering both listings as endpoints:
...
- [ ] **0.2** Lift `seedMember` out of `new-settlement.spec.ts` into `e2etests/utils/exclusions.ts`, adding a
...
- [ ] **1.1** `MembersTabs` component (`src/components/members/MembersTabs.tsx`): two tabs, active state from
...
- [ ] **1.2** Route `/members/excluded` in `App.tsx` behind `ProtectedRoute`; `MainLayout` keeps the Members
  nav item active on both paths.
- [ ] **1.3** Locale keys under a new `excluded` namespace in `public/locales/{en,de}.json` — every visible
  string, per #129. Section titles reuse the preview's phrasing.
- [ ] **1.4** Render `MembersTabs` on `MembersPage` without otherwise touching it.
...
- [ ] **2.1** `useExcludedFromCollection` hook: two independent `useLatestRequest` slots (one per endpoint),
...
- [ ] **2.2** Vitest unit test for the hook, following `useExecutionDateInfo.test.ts`: both loaded, one
...
- [ ] **3.1** `ExcludedFromCollectionPage.tsx` shell: tab strip, page title, loading indicator, error alert.
- [ ] **3.2** Two stat tiles — credit total (orange, negative as it arrives) and held total (red) — using
  `formatPrice`, `tabular-nums`, and the `StatCard` colour variants.
- [ ] **3.3** Credit section: table of member + credit balance, footer total, the "pay by hand" note, and an
  empty state that reads as a result rather than a blank.
- [ ] **3.4** Hold section: member, held-back amount, reason verbatim, held-since (admin's calendar day, per
  the #95 rule), held-by, and the Clear hold action; footer total; empty state.
- [ ] **3.5** Test IDs throughout, per the wireframe's inventory and `patterns/test-ids.md`.
...
- [ ] **4.1** `ConfirmDialog` wired to name the member, the amount going back into the next run, and the
  reason and provenance of the hold.
- [ ] **4.2** `POST .../collection-hold/clear`, then `reload()`. A 409 (someone else cleared it) surfaces
...
- [ ] **5.1** Below the `tablet` breakpoint (`useBreakpoint`), both sections become one card per member —
  name and amount on one line so the page stays skimmable, reason wrapping underneath, Clear hold
  full-width in the card.
- [ ] **5.2** Tiles stack; the tab strip shortens its labels; no horizontal body scroll at 390px.
...
- [ ] **6.1** *The issue's named test*: a member in credit appears in the credit section here **and not**
...
- [ ] **6.2** Clearing a hold is a full round trip: dialog names the member → confirm → row gone, hold total
  dropped, and the member is selectable again on New Settlement.
- [ ] **6.3** A held member appears in the hold section and not the credit section, with the reason text the
  reversal wrote.
- [ ] **6.4** Totals equal the sum of the rows the test seeded (found by id, not by position).
- [ ] **6.5** The tab badge counts both lists.
...
- [ ] **7.2** Clear hold works from the card — the same round trip as 6.2, driven through the mobile layout.
  - **Success**: `npx playwright test --project=admin-mobile --grep "excluded" --workers=4` green.

## Milestone 8 — Page object, fixture, docs

- [ ] **8.1** `e2etests/pages/ExcludedFromCollectionPage.ts` — private locators, public intent methods
  (Pattern 006), exported from `pages/index.ts`.
- [ ] **8.2** `authenticatedExcludedPage` fixture in `fixtures/pageObjects.ts` (Pattern 007).
- [ ] **8.3** Update `plans/INDEX.md`; note the new surface in `admin-frontend/patterns/components.md` if
  `MembersTabs` is reusable.
- [ ] **8.4** Full suite green before the PR: `cd e2etests && npm test -- --workers=4`.
