...
**Status**: In Progress (M1–M3 + M5.1 done and verified; M4 surfacing and M5.2–5.3 open)
...
- [ ] **4.1** Dashboard: `alerts.terminal_anomaly` in `DashboardService::getDashboard()` as a static method beside `encryptionKeyAlert()`; OpenAPI schema; regenerate the TS client; banner on `DashboardPage.tsx` following the existing inline-banner pattern with `data-testid`/`data-severity`.
- [ ] **4.2** Terminals surface: anomaly summary on `TerminalDto`; marker in `TerminalsTab.tsx` and `TerminalCredentials.tsx`.
- [ ] **4.3** Email: `MailKind::TERMINAL_ANOMALY_WARNING`, a `TerminalAnomalyMail` builder + registration in `MailContentRegistry` (today it only carries `SettlementMailBuilder`, so an admin-addressed row would fail to render), `MailStrings` de/en entries, enqueue via `NotificationsService::warnAdmins()`.
- [ ] **4.4** Acknowledge: `POST /api/admin/terminals/{id}/anomalies/{anomalyId}/acknowledge`, audited as `terminal_anomaly_acknowledged`; wired to the UI marker.
