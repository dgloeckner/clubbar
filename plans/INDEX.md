...
| Plan | Status | Summary |
|------|--------|---------|
| [IBAN Encryption (Sealed Boxes)](./2026-08-13-iban-encryption-sealed-box.md) | In Progress (P0–P7 done — crypto foundation, encrypt-on-write, last4-only surfaces, SEPA export via temporary private key, key rotation + Security & Credentials page, terminal-token lifecycle, mandate scans extraction-only with no retention; P8 pending) | Epic [#388](https://github.com/dgloeckner/clubbar/issues/388) / [ADR-0036](../adr/0036-iban-encryption-sealed-box.md) + [ADR-0037](../adr/0037-mandate-documents-not-retained.md) — IBANs encrypted at rest with libsodium sealed boxes (server holds public key only; private key offline with the club, supplied per SEPA export/rotation behind TOTP step-up), 365-day key + terminal-token lifecycle with 90/30/7 warnings, last4-only API surfaces, mandate scans no longer retained |
