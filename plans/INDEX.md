...
| [SEPA Pre-Notification Emails](./2026-08-13-sepa-prenotification-emails.md) | In Progress (P0–P5 done — ADR-0038 + amendments, mail transport/layout/`mail_config`, the `mail_outbox` queue written inside the finalize transaction, the de/en announcement and cancellation content, the drain (`bin/cron.php`, `DrainService`, the `flock`, the URL fallback), and now the gate: **an installation whose scheduler has never been seen to run cannot finalize a direct debit**, says so in a banner on every page and in a sixth installer step, and warns before finalizing about collected members with no email. P6–P10 open; P6's monitoring is next) | Epic [#361](https://github.com/dgloeckner/clubbar/issues/361) / [ADR-0038](../adr/0038-transactional-mail-outbox-on-shared-hosting.md) — announce every SEPA collection by email 7 days ahead (Nutzungsordnung § 7 Abs. 3) via a transactional outbox written inside the finalize transaction and drained by a mandatory scheduler, the only sending path. |
