...
9. **Diagnosing a Red E2E Job in CI**
   ...
   **The CI job runs in three lanes, not two blind shards.** `--shard` splits
   ...
   | `chain` | `api-ordered`, `api-rotation`, `mail-backup`, `mail-chain`, `mail-statement`, `mail-credentials`, `mail-issuance`, `mail-lifecycle`, `mail-jugendschutz`, `mail-roles`, `mail-digest` | 1 job, no shard: every project in it is deliberately serial |
