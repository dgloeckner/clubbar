...
**Implementation Status**: Backend implemented ([#778](https://github.com/dgloeckner/clubbar/issues/778), [#780](https://github.com/dgloeckner/clubbar/issues/780)) — the submission endpoint gates on the poster secret, seals the IBAN, and returns the club's filled Anmeldung in the response itself. The page an applicant's phone renders is implemented too ([#781](https://github.com/dgloeckner/clubbar/issues/781)): a framework-free bundle served from the backend's own document root at `/register/`, with the poster secret read from the URL fragment. **Fully implemented and covered end to end** as of [#784](https://github.com/dgloeckner/clubbar/issues/784): `registration-flow.spec.ts` carries one applicant from the QR scan to a member the terminal can serve, in a browser context holding no admin session.
