 ...
 *
 * The file is generated on first access and deleted after successful
 * installation. Once an admin account exists it is not regenerated by a bare
 * request — only `?update=1` (running the updater on an installed system)
 * mints a fresh one (#884); a scanner that merely touches this script leaves
 * no credential behind and gets the "Already Installed" page for the steps
 * that could otherwise redo the install.
 */
...
// --- Already installed? ---
// Checked before anything else below — the key gate included — so that a
// club with a running installation never even gets a chance to submit a key
// against the steps that can do damage to it (#884).
//
// The real "the wizard is done" signal is an admin account, not merely
// whether config.php exists: that is already true from step 2 onward, in the
// middle of every fresh install. Gating on config.php's existence directly
// would either leave step 2/4 reachable throughout that window (the bug this
// closes) or block the fresh install's own step 4 the moment step 2 finished
// (a new bug). So this asks the database instead, and only for the requests
// that can do damage on a club that is actually running: a bare visit (the
// landing page), step 2 (rewrites the database credentials and can re-mint
// the encryption keys), and step 4 (creates an admin account). Steps 1, 3 and
// 5-8 stay reachable regardless — they are read-only or explicitly
// idempotent, and blocking them would also break the documented `?update=1`
// continuation these same steps serve.
//
// `test_db` and `check_cron` carry no `step` of their own (see install.js) —
// without excluding them by name, the Prüfen button on step 7 of a wizard run
// that has already created its admin would itself get mistaken for a bare
...
//
// Never (re)generated once an admin account exists, unless the operator
// explicitly asked for the updater (#884): the already-installed gate above
// only covers step 2/4/the bare page, but a scanner touching any other step
// on a system whose key file is gone must not leave a fresh one behind
// either — that is what kept the installer permanently re-armed. The same
// admin-account check as above, not the cheaper "does config.php exist":
// a crashed install that wrote config.php but never reached step 4 must
// still be able to recover a key without needing to know about `?update=1`.
$installerData = readInstallerData($dataFile);
if (empty($installerData) && (!installerAdminAccountExists($configFile) || $isUpdate)) {
...
if (empty($_SESSION['install_key_verified'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['install_key'])) {
        ...
        if ($stored !== '' && hash_equals($stored, $provided)) {
            // Rotate the session ID on this privilege transition, the same
            // shape as AuthController::login() (#884). Without it, a session
            // ID an attacker obtained from install.php themselves — planted as
            // a cookie before the operator ever typed the key, the
            // shared-hosting scenario ADR-0025 names — would be sitting there
            // already verified the moment the key is entered.
...
// $step was already read above (the already-installed gate needs it before
// the key gate even runs); default it to '1' now that we know no early exit
// applies.
$step = $step ?? '1';
$error = null;
    ...
    switch ($step) {
        ...
        case '4': // Create admin user
            ...
            try {
                ...
                // Refuse outright once a first admin exists (#884) — reissuing
                // one is what the invitation flow is for, not this wizard. The
                // already-installed gate above refuses this step by the same
                // rule before the request ever reaches here; this is the belt,
                // checked with the same connection right before the write, so
                // two admin-creation requests racing each other cannot both
                // pass a check made moments earlier and both insert a row.
                if ((int) $pdo->query('SELECT COUNT(*) FROM admin_users')->fetchColumn() > 0) {
                    $error = 'An admin account already exists. Use the panel\'s invitation flow to add another.';
                    break;
                }
...
/**
 * Whether the first admin account has already been created (#884) — the
 * actual "this club is running" signal, as opposed to whether config.php
 * exists, which is already true from step 2 onward, in the middle of every
 * fresh install.
 *
 * Deliberately fails closed *for the caller's purposes, not for this
 * function's return value*: anything short of a confirmed row (no
 * config.php yet, a database this host cannot currently reach) comes back
 * `false`, i.e. "not proven installed". That is the safe direction here —
 * every call site uses this to decide whether to *refuse* an action, so
 * "cannot tell" must not itself become a refusal that locks a legitimate
 * install out over a transient database hiccup; step 2/3/4's own handlers
 * already surface a real connection failure to the operator when it matters.
 *
 * Plain PDO, not the application's repositories — Composer's autoloader is
 * out of reach this early (see the file header), the same reason step 4's
 * own admin_users INSERT below does not use one either.
 */
function installerAdminAccountExists(string $configFile): bool
{
    if (!file_exists($configFile)) {
        return false;
    }
