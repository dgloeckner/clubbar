...
| **Type exports** | All request/response types re-exported from `src/api/generated/model/index.ts` — import them from `'…/api/generated/model'` |
...
- `src/api/generated/index.ts` — barrel re-export of the tag factories
- `src/api/generated/model/*.ts` — TypeScript interfaces for all schemas
- `src/api/generated/model/index.ts` — barrel re-export of all types

The schemas live in their own `model/` directory rather than beside the tag
factories, because with both writing into one directory the two barrels raced
for the same `index.ts` and whichever orval wrote last won. orval 8.26 wrote
the schema barrel last, orval 8.28 the tag barrel, and the upgrade silently
took every type export off `src/api/generated/index.ts` (#843). Separate
directories mean neither barrel can overwrite the other.
