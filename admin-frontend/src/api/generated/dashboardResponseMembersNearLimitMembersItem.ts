 ...
 */
import type { DashboardResponseMembersNearLimitMembersItemStatus } from './dashboardResponseMembersNearLimitMembersItemStatus.ts';
...
export type DashboardResponseMembersNearLimitMembersItem = {
  ...
  /** The ceiling **this member** was measured against: their own override where they have one, the club default where they do not. Always greater than zero — an unlimited member is not listed. */
  limit_cents: number;
  /** Share of this row's `limit_cents` the tab uses, as a whole percentage. Not clamped: a tab past the ceiling reports more than 100. */
  percent_of_limit: number;
  /**
     * - approaching: inside the warning band, still served
     * - exceeded: past the ceiling, the terminal refuses the next checkout
     */
