import { IconProps } from '../types'

export function WeizenNewIcon({ size = 20, ...props }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" {...props}>
      <path d="M20,44Q18,36 14,28Q11,20 10,10L38,10Q37,20 34,28Q30,36 28,44Z" fill="#FBBF24" stroke="#92400E" strokeWidth="2.5" strokeLinejoin="round"/>
      <rect x="9" y="6" width="30" height="6" rx="3" fill="white" stroke="#92400E" strokeWidth="2.5"/>
      <line x1="19" y1="44" x2="29" y2="44" stroke="#92400E" strokeWidth="2.5" strokeLinecap="round"/>
      <line x1="31" y1="40" x2="35" y2="16" stroke="#C4934F" strokeWidth="1.5" strokeLinecap="round"/>
      <ellipse cx="35" cy="14" rx="3" ry="5" fill="#FDE68A" stroke="#C4934F" strokeWidth="1.5"/>
      <circle cx="17" cy="30" r="1.5" fill="white" opacity=".7"/>
    </svg>
  )
}
